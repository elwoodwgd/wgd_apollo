...
{
	uart_putchar (WAVE_UART_V2,data); //发送字节'A'
	return data;
}

unsigned char Uart1_Put_Char1(unsigned char DataToSend)
{
	//TxBuffer[count++] = DataToSend;  
        uart_putchar (WAVE_UART_V2,DataToSend);
	return DataToSend;
}
...
void Uart1_Send_AF1(signed int aa,signed int bb,signed int cc,signed int dd,signed int ee,signed int ff,signed int gg,signed int hh)
{
	unsigned char sum = 0;
	count=0;
	sum += Uart1_Put_Char1(0x88);
	sum += Uart1_Put_Char1(0xAF);
	sum += Uart1_Put_Char1(0x1C);
	sum += Uart1_Put_Char1(BYTE1(aa));//1
	sum += Uart1_Put_Char1(BYTE0(aa));
	sum += Uart1_Put_Char1(BYTE1(bb));//2
	sum += Uart1_Put_Char1(BYTE0(bb));
	sum += Uart1_Put_Char1(BYTE1(cc));//3
	sum += Uart1_Put_Char1(BYTE0(cc));
	sum += Uart1_Put_Char1(BYTE1(dd));//4
	sum += Uart1_Put_Char1(BYTE0(dd));
	sum += Uart1_Put_Char1(BYTE1(ee));//5
	sum += Uart1_Put_Char1(BYTE0(ee));
	sum += Uart1_Put_Char1(BYTE1(ff));//6
	sum += Uart1_Put_Char1(BYTE0(ff));
                sum += Uart1_Put_Char1(BYTE1(gg));//7,4500->45'//这是姿态!!!
	sum += Uart1_Put_Char1(BYTE0(gg));
	sum += Uart1_Put_Char1(BYTE1(hh));//8
	sum += Uart1_Put_Char1(BYTE0(hh));
	Uart1_Put_Char1(0);
	Uart1_Put_Char1(0);
	Uart1_Put_Char1(0);
	Uart1_Put_Char1(0);
